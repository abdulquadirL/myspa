...
export default function DashboardPage({
  ...
  userName,
}: { bookings: Booking[]; userName: string }) {
  ...
  const [totalCount, setTotalCount] = useState(initialBookings.length || 0);
  const pageSize = 10;
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <Filters
        ...
        setDateFilter={(dateFilter: string) => handleFiltersChange({ dateFilter })}
        onExportCSV={exportCSV}
        search={filters.search}
        setSearch={(search: string) => handleFiltersChange({ search })}
        statusFilter={filters.statusFilter}
        setStatusFilter={(statusFilter: string) => handleFiltersChange({ statusFilter })}
        serviceFilter={filters.serviceFilter}
        setServiceFilter={(serviceFilter: string) => handleFiltersChange({ serviceFilter })}
      />
